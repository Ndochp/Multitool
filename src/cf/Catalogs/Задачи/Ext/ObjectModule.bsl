...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() и ЗначениеЗаполнено(Родитель) Тогда
		НоваяСсылка = Справочники.Задачи.ПолучитьСсылку(Новый УникальныйИдентификатор);
	...
	Если ЧтоМенять.Свойство("Родитель") 
		Или (ЭтоНовый() и ЗначениеЗаполнено(Родитель))
		Тогда
		ЧтоМенять = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Родитель, КонтролируемыеРеквизитыСтрокой, Истина);

		ДанныеКэша = ДанныеКэша(ЭтотОбъект.Родитель);
		ЧтоМенять.Удалить("Родитель");
		Для Каждого КЗ из ЧтоМенять Цикл
			Если Не ЗначениеЗаполнено(КЗ.Значение) или КЗ.Значение = Ложь Тогда
				ЧтоМенять[КЗ.Ключ] = ДанныеКэша[КЗ.Ключ];
			КонецЕсли;
		КонецЦикла;
		
		ОбновитьКэш(ЧтоМенять, НоваяСсылка);
...
Функция УточнитьЧтоМенять(Знач вхЧтоМенять, Знач ДанныеОбъекта)
	...
	ЧтоМенять = Новый Структура;
	Для Каждого КЗ из вхЧтоМенять Цикл
		Если Не ЗначениеЗаполнено(ДанныеОбъекта[КЗ.Ключ]) или ДанныеОбъекта[КЗ.Ключ] = Ложь Тогда
...
Функция ДанныеКэша(Задача)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КэшированныеРеквизитыРодителейЗадач.Задача КАК Задача,
	|	КэшированныеРеквизитыРодителейЗадач.Решена КАК Решена,
	|	КэшированныеРеквизитыРодителейЗадач.Срок КАК Срок,
	|	КэшированныеРеквизитыРодителейЗадач.КодПроекта КАК КодПроекта,
	|	КэшированныеРеквизитыРодителейЗадач.ПометкаУдаления КАК ПометкаУдаления
	|ИЗ
	|	РегистрСведений.КэшированныеРеквизитыРодителейЗадач КАК КэшированныеРеквизитыРодителейЗадач
	|ГДЕ
	|	КэшированныеРеквизитыРодителейЗадач.Задача = &Задача"
	;
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
