...
////////////////////////////////////////////////////////////////////////////////
// Украшатель текста
// - Преобразует обычную текстовочку в красивый html для отображения.
//   Может быть когда нибудь прикручу маркдаун, а пока базовый простенький набор на регулярках
//
// Например:
...
Функция txt2html(Строка) Экспорт
	Перем Рез;
	Рез = Строка;
	Преобразования = НаборПравил_txt2html();
	Для Каждого Преобразование из Преобразования Цикл 
		Рез = СтрЗаменитьПоРегулярномуВыражению(Рез, Преобразование.РегулярноеВыражение, Преобразование.ПодстрокаЗамены, Истина, Истина);
	КонецЦикла;
	Возврат Рез;
КонецФункции

// Вариант развития https://github.com/jbroadway/slimdown/blob/master/Slimdown.php
///**
...
Функция НаборПравил_txt2html()
	Перем Рез;
	Рез = ИнициализироватьНаборПравил();
	ДобавитьПравило(Рез, "(https?:\/\/(\S)+)(\s)", "<a href=$1>$1</a>$3");
	ДобавитьПравило(Рез, "$", "<br>");
	
	Возврат Рез;
КонецФункции 

Функция ИнициализироватьНаборПравил()
	Перем Рез;
	Рез = Новый ТаблицаЗначений;
	Рез.Колонки.Добавить("РегулярноеВыражение");
	Рез.Колонки.Добавить("ПодстрокаЗамены");
	Возврат Рез;
КонецФункции

Процедура ДобавитьПравило(ТаблицаПравил, РегулярноеВыражение, ПодстрокаЗамены)
	НоваяСтрока = ТаблицаПравил.Добавить();
	НоваяСтрока.РегулярноеВыражение = РегулярноеВыражение;
	НоваяСтрока.ПодстрокаЗамены = ПодстрокаЗамены;
КонецПроцедуры
